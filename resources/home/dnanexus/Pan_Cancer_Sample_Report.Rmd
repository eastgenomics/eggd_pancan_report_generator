...
```{r}
target_list <- gsub("-", "::",params$targetlist)
predicted_fusions <- paste0(unique(fusion_abdriged$X.FusionName)))
...
fusion_abdriged_short_cols <- fusion_abdriged[, !names(fusion_abdriged) %in% c("FUSION_MODEL", "FUSION_CDS","FUSION_TRANSL")]
fusion_abdriged_long_cols <- fusion_abdriged[, c("FUSION_MODEL", "FUSION_CDS","FUSION_TRANSL")]
...
datatable(fusion_abdriged_short_cols, rownames = F,  extensions = c('FixedColumns','Buttons'),
  options = list(
    ...
    buttons = c('copy', 'csv', 'excel'),
    autoWidth = TRUE,
    columnDefs = list(list(width = '300px', targets = c(31,32)))
  )
)
