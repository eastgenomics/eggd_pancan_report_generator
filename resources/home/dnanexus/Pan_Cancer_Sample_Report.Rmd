...
try({
  ...
  for (i in 1:length(fusions)){
    ...
    if (cov_table[i,2] > 10000) {
      cov_table[i,3] <- as.logical("TRUE")
      cov_table[i,4] <- (1/((cov_table[i,2] / 1000)))*2
      # we do the mean coverage divided by 1,000 as that is the difference in SF
      # we need it in fraction so we do 1/SF to get the value to put into the samtools
      # we times it by two because we assume half the reads are from both genes
    }
  }
